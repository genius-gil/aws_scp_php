...
<?php
if(exec("ls -tr /var2/dn2/",$output)){
	for($i = 0; $i < count($output); $i++){
		$j = (int)$i + 1;
		$fname = rawurlencode($output[$i]);
		//echo $fname;
		//echo $j.". ".$output[$i]." <a href=\"/dn/$output[$i]\">Download</a><br>\n";
		$fsize_kb = "(" . round(filesize("/var2/dn2/" . $output[$i])/1024/1024, 2) . " MB)";
		echo "<a href=/dn.php?type=custom&fname=$fname> $j. $output[$i] $fsize_kb</a><br><p>\n";
	}
}
else{
